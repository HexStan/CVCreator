    ...
    <div style={{ fontFamily: fontFamily || '"Helvetica Neue", Arial, sans-serif', maxWidth: innerWidth ? `${innerWidth}px` : undefined }}>
        {!hideBasicInfo && (
          <div style={{ textAlign: 'center', marginBottom: 14 }}>
            ...
            {fields?.filter((f) => f.key === 'name' && f.value).map((f) => (
              <div key={f.key} style={{ fontSize: 24, fontWeight: 300, letterSpacing: 2, marginBottom: 4 }}>{f.value}</div>
            ))}
            {fields?.filter((f) => f.key === 'jobTitle' && f.value).map((f) => (
              <div key={f.key} style={{ fontSize: 13, color: '#888', marginBottom: 6 }}>{f.value}</div>
            ))}
            <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '2px 14px', fontSize: 12, color: '#555' }}>
              {fields?.filter((f) => !['name', 'jobTitle'].includes(f.key) && f.value).map((field) => (
